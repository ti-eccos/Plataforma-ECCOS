...
const ComprasTecnologia = () => {
  ...
  useEffect(() => {
    const checkUnreadMessages = () => {
      ...
      // Considerar apenas solicitações de tecnologia
      const techRequests = allRequests.filter(req => req.type === 'purchase' && req.tipo === "Tecnologia");
  ...
  const handleOpenChat = async (request: RequestData) => {
    try {
      ...
      setViewedRequests(prev => {
        ...
        localStorage.setItem('techViewedRequests', JSON.stringify(Array.from(newSet)));
        return newSet;
      });
  ...
  const filteredRequests = allRequests
    .filter(req => req.type === 'purchase' && req.tipo === "Tecnologia")
    .filter(req => {
      ...
      <div className="min-h-screen bg-white relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-8 p-6 md:p-12">
          ...
          <h1 className="text-3xl font-bold flex items-center gap-2 bg-gradient-to-r from-sidebar to-eccos-purple bg-clip-text text-transparent">
            ...
            Solicitações de Compra - Tecnologia
          </h1>
...
export default ComprasTecnologia;